...
export default function Profile({ user: propUser, setUser }) {
  ...
  useEffect(() => {
    ...
    if (!savedUser) navigate("/login");
      ...
      if (savedUser.profileImage) setPreviewImage(savedUser.profileImage);
    }
  }, [navigate, setUser]);
  ...
  const uploadImageToFirebase = async () => {
    ...
    try {
      const storageRef = ref(storage, `profiles/${Date.now()}_${profileImage.name}`);
      ...
      return new Promise((resolve, reject) => {
        uploadTask.on(
          ...
          (error) => {
            ...
            setMessage("Rasm yuklashda xatolik yuz berdi");
            reject(null);
          },
          async () => {
  ...
  const handleSaveProfile = async (e) => {
    e.preventDefault();
    try {
      let imageUrl = previewImage; // eski rasm bo‘lsa ham saqlab turadi
      if (profileImage) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-indigo-50">
      ...
      <div className="max-w-5xl mx-auto py-10 px-6">
        ...
        {message && (
          <p className="mb-4 bg-yellow-100 text-yellow-800 px-4 py-2 rounded-lg shadow-sm">
            {message}
          </p>
        )}
