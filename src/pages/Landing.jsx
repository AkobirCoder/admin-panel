...
export default function Landing() {
  ...
  return (
    <div className={`${theme === "dark" ? "bg-gray-900 text-gray-100" : "bg-gray-50 text-gray-800"} min-h-screen`}>
      <Helmet>
      ...
      <header className={`${theme === "dark" ? "bg-gray-800" : "bg-white"} shadow-md sticky top-0 z-50`}>
        <div className="px-6 py-4 flex justify-between items-center">
          ...
          <nav className="flex space-x-6 items-center">
            ...
            <Link to="/dashboard" className="hover:text-indigo-600">{t.dashboard}</Link>
            <Link to="/login" className="hover:text-indigo-600">{t.login}</Link>
            <Link to="/register" className="hover:text-indigo-600">{t.register}</Link>
          </nav>
          <div className="flex items-center space-x-4">
            <button
              onClick={toggleLang}
              className="px-3 py-1 border rounded-lg hover:bg-indigo-100 dark:hover:bg-gray-700"
            >
              {lang === "uz" ? "EN" : "UZ"}
            </button>
            <button
      ...
      {/* Features Section */}
      <section className={`${theme === "dark" ? "bg-gray-800 text-black" : "text-black-300"} py-16 bg-gray-100`}>
        <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 px-6">
          {[
            {
              title: "Xizmatlar",
      ...
      {/* Call to Action */}
      <section className={`${theme === "dark" ? "text-white" : "bg-gray-900 text-gray-300"} bg-indigo-600 py-16 text-center text-white`}>
        <h2 className="text-3xl font-bold mb-4">Bugun ro'yxatdan o'ting</h2>
        <p className="mb-6 opacity-90">
          Ro'yxatdan o'tib, tizimning barcha imkoniyatlaridan foydalaning.
        </p>
      ...
      {/* Footer */}
      <footer className={`${theme === "dark" ? "bg-gray-800 text-gray-400" : "bg-gray-900 text-gray-300"} py-8`}>
        <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center">
          <p className="text-sm">
            © {new Date().getFullYear()} {t.rights}
          </p>
          <div className="flex space-x-6 mt-4 md:mt-0">
            <Link to="/dashboard" className="hover:text-white">{t.dashboard}</Link>
