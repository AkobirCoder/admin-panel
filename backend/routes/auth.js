...
const RECAPTCHA_SECRET = "6LdyGukrAAAAAOcKh9zXz9fYkDkKglldva27F8G5";
...
router.post("/login", async (req, res) => {
  try {
    ...
    if (!captchaToken) {
      return res.status(400).json({ success: false, message: "Captcha token topilmadi" });
    }
    ...
    if (!response.data.success) {
      return res.status(400).json({ success: false, message: "Captcha noto'g'ri yoki muddati o'tgan" });
    }
